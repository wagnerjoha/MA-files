...
plot1 <- ggplot(cov_df, aes(x = size, y = differences, fill = method)) +
  geom_boxplot() +
  theme_minimal() +
...
plot2 <- ggplot(cov_df_auto, aes(x = size, y = differences, fill = method)) +
  geom_boxplot() +
  theme_minimal() +
  scale_fill_manual(values = set_1) +
  labs(y = "dist covariance",
...
grid.arrange(plot1, plot2, ncol = 2, top = textGrob("Distance of true covariance to estimated covariance", gp = gpar(fontsize = 16)))
dev.off()
