...
cov_df_auto$method <- factor(cov_df_auto$method, 
                        levels = c("MCEM", "standard with GBM", "robust with GBM"))

cov_df_auto$size <- factor(cov_df_auto$size, 
                        levels = c("cts per sec", "cts per dsec", "cts per csec"))
...
png("./scripts/figures/figure_10_upd.png", width = 12, height = 5, units = "in", res = 300)
