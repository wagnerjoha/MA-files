...
sigma_diff_df <- data.frame(
  ...
  method = factor(rep(methods, each = length(methods) * n_simulations)),
...
#********** autocorrelated scores **********#
simulation_data_list <- list(
  tar_read(sim_complex_sec_auto),
  tar_read(sim_complex_dsec_auto),
  tar_read(sim_complex_csec_auto)
)
